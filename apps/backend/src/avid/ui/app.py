    ...
    with gr.Blocks(title="AVID - Auto Video Edit") as app:
                ...
                with gr.Row():
                    with gr.Column():
                        ...
                        stage_transcribe = gr.Checkbox(
                            label="음성 인식 (Chalna)",
                            value=True,
                            info="Chalna API를 사용하여 음성을 텍스트로 변환합니다",
                        )
                        stage_silence = gr.Checkbox(
                            label="무음 구간 감지",
                            value=True,
                            info="자막 갭 기반으로 무음 구간을 감지합니다",
                        )
                    ...
                    with gr.Column():
                        ...
                        with gr.Accordion("음성 인식 옵션 (Chalna)", open=False):
                            transcribe_language = gr.Dropdown(
                                label="언어",
                                choices=[("한국어", "ko"), ("영어", "en"), ("일본어", "ja"), ("자동 감지", "auto")],
                                value="ko",
                                info="음성 인식 언어 선택",
                            )
                            transcribe_use_alignment = gr.Checkbox(
                                label="Qwen2 정렬 사용",
                                value=True,
                                info="더 정확한 타임스탬프 정렬 (권장)",
                            )
                            transcribe_use_llm = gr.Checkbox(
                                label="LLM 정제 사용",
                                value=False,
                                info="텍스트 품질 개선 (처리 시간 증가)",
                            )
                            transcribe_context = gr.Textbox(
                                label="컨텍스트 (선택사항)",
                                placeholder="예: 기술 관련 영상, 요리 레시피 등",
                                info="전사 정확도 향상을 위한 힌트",
                            )
        ...
        def run_pipeline(
            ...
            if transcribe:
                ...
                stage_options["transcribe"] = {
                    "language": transcribe_lang,
                    "use_alignment": transcribe_alignment,
                    "use_llm_refinement": transcribe_llm,
                    "context": transcribe_ctx if transcribe_ctx else None,
                }

            if silence:
                stages.append("silence")
                stage_options["silence"] = {
                    "min_silence_ms": silence_ms,
                }

            if duplicate:
                stages.append("duplicate")
                stage_options["duplicate"] = {
            ...
            for i, stage in enumerate(stages):
                ...
                opts = stage_options.get(stage, {})
                log_lines.append(f"[INFO] Stage '{stage}' - 옵션: {opts}")
