...
from _common import SubtitleSegment, call_claude, parse_json_response, format_filtered_context_for_prompt, format_podcast_context_for_prompt, process_chunks_parallel
...
def _edit_intensity_guidance(edit_intensity: str = "normal") -> str:
    ...
    details = {
        ...
        "heavy": [
            ...
            "- 맥락 연결용 bridge segment는 앞뒤를 합쳐 읽었을 때 setup/payoff나 문장 완성에 필요한지 판단하세요.",
        ],
    }
    lines = [
        "## 컷 편집 강도 지시 (최우선)",
...
9. **메타 발언 (meta_comment)**: 촬영·녹화·장비 관련 대화, 콘텐츠 기획 논의, "시작하겠습니다" 같은 준비 멘트. 시청자에게 보여줄 본 내용이 아닌 제작 과정 대화.
...
- 편집 강도 지시를 우선하세요. 강도가 높을수록 더 적극적으로 압축하되, 남은 대화가 자연스럽게 이어져야 합니다.
