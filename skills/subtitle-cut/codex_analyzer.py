...
## 중복이 아닌 것 (주의! — 이것들을 duplicate로 자르면 안 됩니다):
- **나열형 설명**: "A 따로, B 따로, C가 아니라" 같은 목록의 각 항목은 하나의 논점을 구성하는 부분이지 중복이 아닙니다.
- **짧은 정의·요약**: "이게 X입니다", "P가 Y예요" 같은 한 줄 정의는 독립적 정보입니다. 같은 주제 안에서 다른 용어/개념을 정의하는 것은 중복이 아닙니다.
- **보충 설명**: 같은 주제를 다른 각도에서 설명하는 것은 중복이 아니라 보충입니다.
- **최종 테이크 내의 문장**: 3단계에서 최종 테이크로 선택한 구간 안의 모든 문장은 유지하세요. 같은 주제를 다뤘다고 해서 최종 테이크 내의 개별 문장을 자르면 안 됩니다.
- **결론·요약문**: "핵심은 ~이다", "~법칙이죠", "결국 ~인 셈이죠" 같은 마무리 문장은 핵심 정보입니다.
